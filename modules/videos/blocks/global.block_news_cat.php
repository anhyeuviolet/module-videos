...
if( ! nv_function_exists( 'nv_block_news_cat' ) )
{
	...
	function nv_block_news_cat( $block_config )
	{
		...
		if( ! empty( $list ) )
		{
			if( file_exists( NV_ROOTDIR . '/themes/' . $global_config['module_theme']  . '/modules/news/block_news_cat.tpl' ) )
			{
			...
			$xtpl = new XTemplate( 'block_news_cat.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/news' );
			foreach( $list as $l )
			{
